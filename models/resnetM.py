...
if __name__ == '__main__':
    ...
    m_name = ['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152']
    for i, net_fn  in enumerate([resnet18, resnet34, resnet50, resnet101, resnet152]):

        model = net_fn(pretrained=False).cuda()
        print("\nNetwork:",m_name[i] )
        print("parameters number:", sum(p.numel() for p in model.parameters()),
                "\ntrainable parameters number:", sum(p.numel() for p in model.parameters() if p.requires_grad))
        output = model(input)
        print("input size:", input.size())
        for i, o in enumerate (output):
            print("layer % d:"% (i+1), o.size())
